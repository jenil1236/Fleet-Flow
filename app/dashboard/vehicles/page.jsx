...
import { Plus, Search, Truck, Edit, Power } from 'lucide-react';
...
function VehicleCard({ vehicle, onUpdate }) {
  ...
  const handleToggleService = async () => {
    if (vehicle.status === 'ON_TRIP') {
      alert('Cannot change status while vehicle is ON_TRIP');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`/api/vehicles/${vehicle.id}/toggle-service`, {
        method: 'PATCH',
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs mb-4">
        ...
        <div>
          ...
          <p className="text-gray-900 dark:text-white font-medium">${Number(vehicle.acquisitionCost).toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
        ...
        <button
          ...
          disabled={toggling || vehicle.status === 'ON_TRIP'}
        ...
        >
          ...
          {toggling ? '...' : vehicle.status === 'OUT_OF_SERVICE' ? 'Activate' : 'Deactivate'}
        </button>
      </div>
    </div>
  );
