        ...
        {filteredLogs.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredLogs.map(log => (
              <MaintenanceCard key={log.id} log={log} onUpdate={fetchMaintenance} />
            ))}
          </div>
        )}
      </div>
...
function MaintenanceCard({ log, onUpdate }) {
  ...
  const handleClose = async () => {
    if (!confirm('Are you sure you want to close this maintenance log? The vehicle will be marked as AVAILABLE.')) {
      return;
    }

    setClosing(true);
    try {
      const response = await fetch(`/api/maintenance/${log.id}/close`, {
        method: 'PATCH',
      });
    ...
    <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
      ...
      {log.status !== 'CLOSED' && (
        <button
          onClick={handleClose}
          disabled={closing}
          className="w-full mt-3 px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white rounded-lg text-sm font-medium transition-colors"
        >
          {closing ? 'Closing...' : 'Close Maintenance'}
        </button>
      )}
    </div>
  );
